...
/// §1: SSE streaming writer — processes SSE lines incrementally as the HTTP
/// response arrives via client.fetch(). Each write() call from the HTTP layer
/// feeds partial data; we buffer until we see '\n' and dispatch events on blank lines.
const SseStreamWriter = struct {
    alloc: std.mem.Allocator,
    ...
    fn init(alloc: std.mem.Allocator) SseStreamWriter {
        return .{
            .alloc = alloc,
            .line_buf = .{},
            .current_data = null,
    ...
    const Writer = std.io.GenericWriter(*SseStreamWriter, error{}, write);
    fn writer(self: *SseStreamWriter) Writer {
        return .{ .context = self };
    }

    fn write(self: *SseStreamWriter, bytes: []const u8) error{}!usize {
        for (bytes) |b| {
            if (b == '\n') {
...
fn streamRunOutput(
    ...
    var sse = SseStreamWriter.init(alloc);
    defer sse.deinit();
    ...
    const result = try client.fetch(.{
        ...
        .response_writer = &sse.writer().any(),
    });

...
test "canonical schema bootstrap includes scoring config in base schema" {
    ...
    try std.testing.expect(std.mem.containsAtLeast(u8, migrations[7].sql, 1, "trust_streak_runs INTEGER NOT NULL"));
    try std.testing.expect(std.mem.containsAtLeast(u8, migrations[7].sql, 1, "trust_level   TEXT NOT NULL"));
    try std.testing.expect(!std.mem.containsAtLeast(u8, migrations[7].sql, 1, "trust_streak_runs INTEGER NOT NULL DEFAULT 0"));
    try std.testing.expect(!std.mem.containsAtLeast(u8, migrations[7].sql, 1, "trust_level   TEXT NOT NULL DEFAULT 'UNEARNED'"));
    try std.testing.expect(std.mem.containsAtLeast(u8, migrations[7].sql, 1, "CREATE TABLE agent_improvement_proposals"));
    try std.testing.expect(std.mem.containsAtLeast(u8, migrations[7].sql, 1, "generation_status   TEXT NOT NULL"));
    try std.testing.expect(!std.mem.containsAtLeast(u8, migrations[7].sql, 1, "CHECK (trigger_reason IN"));
    try std.testing.expect(!std.mem.containsAtLeast(u8, migrations[7].sql, 1, "CHECK (approval_mode IN"));
    try std.testing.expect(!std.mem.containsAtLeast(u8, migrations[7].sql, 1, "CHECK (generation_status IN"));
    try std.testing.expect(!std.mem.containsAtLeast(u8, migrations[7].sql, 1, "CHECK (status IN"));
    try std.testing.expect(!std.mem.containsAtLeast(u8, migrations[7].sql, 1, "WHERE status ="));
    try std.testing.expect(std.mem.containsAtLeast(u8, migrations[7].sql, 1, "CREATE TABLE harness_change_log"));

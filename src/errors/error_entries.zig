...
pub const ENTRIES = [_]Entry{
    ...
    e("UZ-INTERNAL-003", .internal_server_error, "Internal operation failed", "An internal operation failed. Check the err= field for details. " ++
    ...
    e("UZ-WORKSPACE-003", .payment_required, "Workspace free limit reached", "Free tier workspace limit reached. Upgrade your plan."),
    // ── BILLING ──────────────────────────────────────────────────────────────
    e("UZ-BILLING-001", .bad_request, "Invalid subscription ID", "The subscription ID is invalid or malformed."),
    e("UZ-BILLING-002", .internal_server_error, "Billing state missing", "Billing state record is missing. Contact support."),
    e("UZ-BILLING-003", .internal_server_error, "Billing state invalid", "Billing state is in an invalid state. Contact support."),
    e("UZ-BILLING-004", .bad_request, "Invalid billing event", "The billing event payload is invalid."),
    e("UZ-BILLING-005", .payment_required, "Credit exhausted", "Workspace credits are exhausted. Add credits or upgrade plan."),
    // ── SCORING ──────────────────────────────────────────────────────────────
    e("UZ-SCORING-001", .bad_request, "Invalid scoring context", "Scoring context tokens are invalid. Check the context_tokens field."),
    // ── ENTITLEMENT ──────────────────────────────────────────────────────────
    e("UZ-ENTL-001", .service_unavailable, "Entitlement service unavailable", "Entitlement service is temporarily unavailable. Retry shortly."),
    e("UZ-ENTL-003", .payment_required, "Stage limit reached", "Stage limit reached for your plan. Upgrade to add more."),
    e("UZ-ENTL-004", .forbidden, "Skill not allowed", "This skill is not allowed on your current plan."),
    // ── PIPELINE V1 REMOVED ─────────────────────────────────────────────────
    e("UZ-RUNS-410", .gone, "Pipeline v1 permanently removed", "Pipeline v1 has been permanently removed. All /v1/runs/* and /v1/specs " ++
    ...
    e("UZ-WH-010", .unauthorized, "Invalid webhook signature", "Slack signature verification failed. Check that the signing secret " ++
        "in the vault matches the one in Slack App settings."),
    e("UZ-WH-011", .unauthorized, "Stale webhook timestamp", "Slack request timestamp is more than 5 minutes old. " ++
        "This may indicate a replay attack or clock skew."),
    // ── TOOL ─────────────────────────────────────────────────────────────────
    e("UZ-TOOL-005", .bad_request, "Unknown tool", "Unknown tool name. Check spelling against the known skills list."),
    // ── ZOMBIE ───────────────────────────────────────────────────────────────
    ...
    e("UZ-ZMB-007", .bad_request, "Zombie credential value too long", "Credential value exceeds 4KB. Check that the secret is not corrupted or padded."),
    e("UZ-ZMB-008", .bad_request, "Invalid zombie config", "Config JSON is malformed. Verify trigger, skills, credentials, and budget fields. " ++
    ...
    e("UZ-STARTUP-004", .internal_server_error, "Redis connect failed", "Redis is unreachable. Check that REDIS_URL_API and REDIS_URL_WORKER are set " ++
    ...
    e("UZ-EXEC-002", .internal_server_error, "Stage start failed", "Stage failed to start. Check runner configuration."),
    ...
    // ── CREDENTIALS ──────────────────────────────────────────────────────────
    e("UZ-CRED-001", .service_unavailable, "Anthropic API key missing", "Workspace LLM API key not found in vault.secrets (key: anthropic_api_key). " ++
    ...
    e("UZ-CRED-003", .service_unavailable, "Platform LLM key missing", "No active platform LLM key for this provider. Admin must set one via " ++
    ...
    // ── SLACK PLUGIN (M8_001) ────────────────────────────────────────────────
    e("UZ-SLACK-001", .forbidden, "Slack OAuth state invalid", "OAuth state parameter mismatch — possible CSRF attempt. " ++
        "Please try installing UseZombie again from the beginning."),
    e("UZ-SLACK-002", .bad_gateway, "Slack token exchange failed", "Could not exchange OAuth code for bot token. Verify SLACK_CLIENT_SECRET " ++
        "is set and the code has not expired (codes are single-use, 10-minute TTL)."),
    e("UZ-SLACK-003", .unauthorized, "Slack bot token expired", "The Slack bot token has been revoked or expired. " ++

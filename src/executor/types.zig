...
/// Per-execution network egress policy. Outbound HTTPS requests must match
/// at least one entry in `allow` (exact hostname match — wildcard support
/// is a follow-up). Empty `allow` means deny-all egress for this session.
pub const NetworkPolicy = struct {
    allow: []const []const u8 = &.{},
};

/// Context-budget knobs from `x-usezombie.context`. `tool_window == 0` is
/// the sentinel for "auto" — resolved against
/// `executor/registry/model_registry.defaultToolWindow` at use time.
/// `model` is the resolved model name; the same registry maps it to a
/// context cap for L3 fill estimation.
...
/// invariant for the session's lifetime. Stored on the session, threaded
/// through every tool invocation. Empty defaults are deny-all egress,
/// no tool restriction, no secrets, default context budget — so tests
...
pub const ExecutionPolicy = struct {
    ...
    /// names and whose values are the parsed JSON bodies stored in vault.
    /// `null` is "no secrets". The substitution path looks up
    /// `${secrets.NAME.FIELD}` against this object at outbound-request time.

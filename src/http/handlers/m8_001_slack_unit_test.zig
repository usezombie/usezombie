...
test "M8_001: UZ-SLACK-001 (OAuth state invalid) → HTTP 403 Forbidden" {
...
test "M8_001: UZ-SLACK-002 (token exchange failed) → HTTP 502 Bad Gateway" {
...
test "M8_001: UZ-SLACK-003 (bot token expired) → HTTP 401 Unauthorized" {
...
test "M8_001: UZ-WH-010 (invalid Slack signature) → HTTP 401 Unauthorized" {
    // Bad signature → request is not from Slack. 401 chosen over 403 because
    // the identity (it being Slack) cannot be confirmed.
    var ht = httpz.testing.init(.{});
    defer ht.deinit();
    common.errorResponse(ht.res, ec.ERR_WEBHOOK_SLACK_SIG_INVALID, "bad sig", "r4");
    try ht.expectStatus(401);
}

test "M8_001: UZ-WH-011 (stale timestamp) → HTTP 401 Unauthorized" {
    // Replay attack / too-old timestamp. Same 401 family as signature failure —
    // we cannot confirm the request is fresh from Slack.
    var ht = httpz.testing.init(.{});
    defer ht.deinit();
    common.errorResponse(ht.res, ec.ERR_WEBHOOK_SLACK_TIMESTAMP_STALE, "old ts", "r5");
    try ht.expectStatus(401);
}

// ── T7: Constant regression pins ─────────────────────────────────────────────

test "M8_001: SLACK_MAX_TS_DRIFT_SECONDS is exactly 300 (5-minute window)" {
...
test "M8_001: SLACK_SIG_VERSION is \"v0\" (Slack signing protocol version)" {
...
test "M8_001: GATE_PENDING_TTL_SECONDS is at least 3600 (1-hour approval window)" {
...
test "M8_001: GATE_PENDING_KEY_PREFIX contains \"gate\" (routing invariant)" {
...
test "M8_001: SLACK_SIG_HEADER is lowercase (HTTP header comparison must be case-insensitive)" {
...
test "M8_001: SLACK_TS_HEADER is lowercase" {
    for (ec.SLACK_TS_HEADER) |c| {
        try std.testing.expect(c != std.ascii.toUpper(c) or c == '-');
    }
}

test "M8_001: error response body contains error_code field (RFC 7807 contract)" {
...
test "M8_001: error response Content-Type is application/problem+json (RFC 7807)" {
    var ht = httpz.testing.init(.{});
    defer ht.deinit();

...
// ── §5 — body-field PATCH advances the revision the worker reload reads ──

test "integration: PATCH revision is monotonic — reload reads the post-PATCH value" {
    // End-to-end seam: after a body-field PATCH commits, the §10a
    // reloadZombieConfig path SELECTs core.zombies.updated_at and writes
    // a `cfg-<revision>` event row. Asserting the revision the PATCH
    // returned is the one the next reload uses proves §10b → §10a wiring
    ...
    // Drive the §10a path directly off the pool the handler used.
    var session = try event_loop.claimZombie(ALLOC, ZOMBIE_ID, h.pool);
    defer session.deinit(ALLOC);
    try event_loop.reloadZombieConfig(ALLOC, &session, h.pool);

    const c = try h.acquireConn();
    defer h.releaseConn(c);

    // Read back the system row — event_id must match cfg-<after.updated_at>.
    const expected_event_id = try std.fmt.allocPrint(ALLOC, "cfg-{d}", .{after.updated_at});
    defer ALLOC.free(expected_event_id);
    var q = PgQuery.from(try c.query(
        \\SELECT event_id FROM core.zombie_events
        \\WHERE zombie_id = $1::uuid AND actor = 'system:config_update'
        \\ORDER BY event_id DESC LIMIT 1
    , .{ZOMBIE_ID}));
    defer q.deinit();
    const row = (try q.next()) orelse return error.SystemRowMissing;
    const event_id = try row.get([]const u8, 0);
    try std.testing.expectEqualStrings(expected_event_id, event_id);

...
type SteerOutcome =
  ...
  | { kind: "sse_error"; err: unknown; detail?: string };
...
export async function commandSteer(
  ctx: CommandCtx,
  parsed: ParsedArgs,
  ...
  const streamGet: StreamGetFn = deps.streamGet ?? defaultStreamGet;
  ...
  let outcome: SteerOutcome = await tailEventStream(
    ctx,
    wsId,
    zombieId,
    eventId,
    deps,
    streamGet,
  ).catch((err: unknown): SteerOutcome => ({ kind: "sse_error", err }));
  ...
  if (ctx.jsonMode && ctx.stdout) {
    ...
    if (ctx.stderr)
      writeLine(
        ctx.stderr,
        ui.err(`event ${eventId} still in flight after ${Math.round(SSE_FALLBACK_TIMEOUT_MS / 1000)}s — check: zombiectl events ${zombieId}`),
      );
  } else if (ctx.stderr) {
    const detail = outcome.kind === "sse_error" && outcome.detail ? ` — ${outcome.detail}` : "";
    writeLine(
      ctx.stderr,
      ui.err(`message failed: ${outcome.kind}${detail}`),
    );
  }

  if (outcome.kind === "complete") {
    return outcome.status === EVENT_STATUS.PROCESSED ? 0 : 1;
  }
  return 1;
}
...
async function tailEventStream(
  ...
  await streamGet(url, headers, (event) => {
    ...
    if (frameEventId && frameEventId !== eventId) return undefined;
    if (event.type === "chunk" && typeof payload["text"] === "string" && ctx.stdout) {
      writeLine(ctx.stdout, `${ui.dim("[claw]")} ${payload["text"] as string}`);
      return undefined;
    }
    if (event.type === "tool_call_started" && typeof payload["name"] === "string" && ctx.stdout) {
      writeLine(ctx.stdout, `${ui.dim("[tool]")} ${payload["name"] as string} starting`);
      return undefined;
    }
    if (event.type === "tool_call_completed" && typeof payload["name"] === "string" && ctx.stdout) {
      const ms = typeof payload["ms"] === "number" ? `${payload["ms"] as number}ms` : "";
      writeLine(ctx.stdout, `${ui.dim("[tool]")} ${payload["name"] as string} done ${ms}`);
      return undefined;
    }
    if (event.type === "event_complete") {
      const status = typeof payload["status"] === "string" ? (payload["status"] as string) : "unknown";
      outcome = { kind: "complete", status };
...
async function pollEventTerminal(
  ...
  while (Date.now() < deadline) {
    ...
    }).catch(() => null)) as { items?: EventRow[] } | null;
    const match = res?.items?.find((row) => row.event_id === eventId);
    if (match && match.status && isTerminal(match.status)) {
...
function eventIdToSince(eventId: string): string | null {
...
function buildBearer(ctx: CommandCtx): Record<string, string> {

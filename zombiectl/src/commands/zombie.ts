...
> = Effect.gen(function* () {
  ...
  if (zombies.length === 0) {
    yield* output.info(
      "No zombies running. Install one with: zombiectl install --from <path>",
    );
    return;
  }
...
> =>
  Effect.gen(function* () {
    ...
    const wsId = yield* requireWorkspaceId;
    const id = yield* requireZombieId(zombieId, `zombiectl ${verb} <zombie_id>`);
    const token = yield* resolveAuthToken;
...
> =>
  Effect.gen(function* () {
    ...
    const http = yield* HttpClient;
    const wsId = yield* requireWorkspaceId;
    const id = yield* requireZombieId(zombieId, "zombiectl delete <zombie_id>");
    const token = yield* resolveAuthToken;

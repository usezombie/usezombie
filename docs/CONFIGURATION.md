...
| `POSTHOG_KEY` | Optional | Process env (optional dev `.env.local` fallback), no CLI | Required when PostHog is enabled. |
| `POSTHOG_HOST` | Optional | Process env (optional dev `.env.local` fallback), no CLI | PostHog host URL. |

...
- **Stat-card primitive vs `<StatusCard>`.** Open question: Mockup B's stat-card pattern (mono uppercase label + mono display value + mono delta) may or may not match design-system's existing `<StatusCard>`. The implementing agent audits `<StatusCard>` at PLAN; if it's a fit, use it. If it isn't, the agent may add a `<Stat>` primitive to design-system as a small in-scope follow-up — this is the **only** sanctioned design-system edit in W3 and requires an explicit Discovery entry + the user's go-ahead before crossing the package boundary.
- **Dashboard landing composition.** Mockup B shows the stat row + something below — implementing agent picks (a) condensed zombies preview or (b) recent-events strip after re-reading the mockup, and documents the choice here.

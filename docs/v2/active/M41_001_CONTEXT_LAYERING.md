...
| `src/executor/context_budget.zig` | NEW | Per-execution policy bundle: `NetworkPolicy`, `ContextBudget` (tool_window, memory_checkpoint_every, stage_chunk_threshold, model), `ExecutionPolicy`. Extracted from types.zig to keep both files under the 350-line gate. **No compile-time model table, no model→cap dictionary** — the executor never has to know which model has which context window; slice §6 picks how to interpret `stage_chunk_threshold` (ratio vs absolute) when it actually implements L3, against whatever signal it ends up using. |
...
ExecutionContextBudget {
  ...
  model:                     []const u8,    // resolved from zombie config (passthrough only)
}
```
...
Before each tool call, check whether the stage has crossed `stage_chunk_threshold`. The threshold is a single `f32` knob on `ContextBudget` (default 0.75); the implementer of this slice picks the signal it's compared against — absolute tokens, fraction of an explicit cap from the zombie's frontmatter, wall-clock budget, or count of large-result tool calls. **What the executor does NOT carry: a platform-side model→cap dictionary.** When the threshold trips, nudge the agent: *"Context approaching budget. Snapshot findings now and return for continuation."* If the next tool call would still exceed, force the agent to call `memory_store` and return `{exit_ok: false, checkpoint_id: <auto>, content: <agent's final reasoning>}`. Worker re-enqueues a continuation event (§7).
...
4. **Model context cap.** Briefly landed as a compile-time `const` model→cap table; deleted Apr 29, 2026 along with the `Tier`/`tierFor`/`defaultToolWindow` helpers and a `context_cap_tokens` field on `ContextBudget`. Reasons: (a) provider release cadence beats our release train — the table rots in days, (b) BYOK (M48) lets customers bring providers we've never heard of, (c) the executor doesn't actually need to know the cap to enforce L3 — `stage_chunk_threshold` is just a knob whose interpretation belongs in slice §6. M41 stays model-agnostic: cap as a concept doesn't exist in this codebase.

...
**Status:** IN_PROGRESS — foundation + the egress **policy abstraction** merged to `main`; the strict (kernel-enforced) implementation behind it lands in 2.0.1 (Indy, Jun 11, 2026). `RUNNER_NETWORK_POLICY` now selects between two egress implementations; see the status map + the validated option-D recipe in Discovery.
> **Policy abstraction merged; strict enforcement in 2.0.1 (Indy, Jun 11, 2026).** Indy: _"its abstraction for me... keep the strict implement (not fully done invoked) when i turn on the option"_ + _"i want the default to be allow_all."_ `RUNNER_NETWORK_POLICY` selects, with **honest names** (each says what it does): `allow_all` (**DEFAULT** — unset resolves here; re-shares host netns via `--share-net` = full egress; the unenforced interim) · `deny_all` (no network) · `registry_allowlist` (**opt-in, STRICT** kernel-enforced option-D path — the name is finally *true*; **fails closed `UZ-RUN-007`** until `establishEgress` is wired in 2.0.1). **⚠️ Security posture:** this is **open-by-default** (was secure-by-default `deny_all`) — the accepted pre-enforcement interim; flip the default back to the enforced posture when 2.0.1 lands. Env-configured (no config file — Indy: env is fine).
...
| **Policy abstraction** | `RUNNER_NETWORK_POLICY` → `Policy.Mode` strategy with honest names: `allow_all` (DEFAULT, full egress) / `deny_all` (no net) / `registry_allowlist` (opt-in strict, fail-closed until D); `sharesHostNet`/`enforcesEgress` helpers; sandbox_args + supervisor wired | 🟢 |

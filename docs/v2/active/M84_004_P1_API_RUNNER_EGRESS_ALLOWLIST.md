...
> **Policy abstraction merged; strict enforcement in 2.0.1 (Indy, Jun 11, 2026).** Indy: _"its abstraction for me... keep the strict implement (not fully done invoked) when i turn on the option. In other cases i call another method or policy."_ `RUNNER_NETWORK_POLICY` selects: `deny_all` (no network) · `registry_allowlist` (**INTERIM allow-all** — re-shares host netns via `--share-net`; the network tier works today, allowlist advisory/L7-only) · `registry_allowlist_strict` (**opt-in** kernel-enforced option-D path; **fails closed `UZ-RUN-007`** until `establishEgress` is wired in 2.0.1). **Merging this is runtime-safe:** `registry_allowlist` behaves exactly like today (allow-all), so nothing breaks; flip to `registry_allowlist_strict` to test enforcement once 2.0.1 lands it. Env-configured (no config file — Indy: env is fine).
...
| **Policy abstraction** | `RUNNER_NETWORK_POLICY` → `Policy.Mode` strategy (deny_all / registry_allowlist=interim allow-all / registry_allowlist_strict=opt-in, fail-closed until D); `sharesHostNet`/`enforcesEgress` helpers; sandbox_args + supervisor wired | 🟢 |

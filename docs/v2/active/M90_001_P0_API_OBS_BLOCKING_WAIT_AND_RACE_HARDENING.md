...
- Jun 10, 2026 — **Pre-existing suite flake identified (not this branch):** `observability/telemetry_test.zig:84` asserts last-event on the process-global `TestBackend` while concurrently-running server tests capture `.server_started` — intermittent "expected .worker_started, found .server_started" / "expected 5, found 1". Stash-bisected: reproduces without this branch's changes. The owning cluster (`WorkerStarted` + telemetry mocks) is in M90_003 §2's deletion inventory; Indy decides interim fix vs ride-until-003.

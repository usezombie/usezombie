...
- **D40 — `src/program/**` migration.** DONE.
  ...
  - **Captain-spotted dedup during migration (validate.ts ↔ validators.ts):** both modules carried a private `EXAMPLE_UUIDV7` literal and duplicated the uuidv7 type-check (`validate.ts:isValidId` is the handler-side type guard; `validators.ts:parseIdOption` is the commander parser). `validate.ts` now exports `EXAMPLE_UUIDV7`; `validators.ts:parseIdOption` calls `isValidId(value)` from `validate.ts` and drops its direct `uuid` import. Two surfaces (parser throws `InvalidArgumentError`, type-guard returns boolean) but one impl + one literal. UFS GATE concern absorbed pre-emptively.

...
| SCHEMA | yes | three migrations: rename `status`→`admin_state` (+ expand values, app-enforced); add `fleet.runner_events` (with the offline-event partial unique index for cross-replica sweeper single-flight); add the `fleet.runner_leases (runner_id, status)` index (sweeper lookup + derived liveness counts). Update `schema/embed.zig` + array. |
...
| `schema/0NN_runner_leases_runner_idx.sql` | CREATE | Index `fleet.runner_leases (runner_id, status)` — the sweeper's "find this runner's active leases" query + the derived `busy`/`active` counts scan by `runner_id`, which is unindexed today. |

...
> **Amendment — Jun 1, 2026:** `HEARTBEAT_LAPSE_MS` was **removed** from `constants.zig` (added in the `ab7da4a8` foundation, never consumed). With §2 deferred there is no reader, and the correct threshold depends on this scan's design — notably whether actively-renewing leases are even in scope (their kill-deadline already bounds recovery to ≤`LEASE_TTL_MS`, so they may not belong in the lapse path at all). The constant is reintroduced **with** §2, derived from the real detection model instead of guessed ahead of it. Greptile flagged the placeholder value as unsafe for the deferred consumer; removing the unconsumed constant resolves it at the root (RULE NDC/NLG).
>
> Indy (2026-06-01): "remove HEARTBEAT_LAPSE_MS now — yes go ahead" — context: chose removal over guessing a value or building a liveness producer ahead of the consumer.

Delivers fast recovery: when a runner's heartbeat lapses (`last_seen_at` older than the §2 lapse threshold), mark it dead and make its leases reclaimable *now* rather than waiting out the TTL. **Mechanism (there is no sweep to speed up — reclaim is pull-triggered):** detection piggybacks on existing traffic — a surviving runner's poll/heartbeat opportunistically scans for lapsed peers and **expires their affinity slots only** (`leased_until := now`), leaving the `active` lease rows intact so the next poll's `reclaimPriorActive` recovers the event durably with no re-bill. "Within one detection cycle" = one poll interval (`NO_WORK_RETRY_AFTER_MS`), well under the TTL. The pull-triggered claim is the backstop for a runner that vanishes silently. The lapsed host is excluded from candidate selection, so work moves to other healthy hosts.
...
- ✅ **Foundation** (`ab7da4a8`): `constants.zig` (`RENEWAL_WINDOW_MS`, `MAX_RUNTIME_MS`); `UZ-RUN-009/010/011/012` + runner mirrors. _(`HEARTBEAT_LAPSE_MS` was added here too but later removed as premature — see the §2 amendment above.)_

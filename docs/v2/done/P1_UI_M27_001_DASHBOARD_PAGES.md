...
- 13.5 DONE — Same-origin `/backend` proxy: `next.config.ts` rewrites `/backend/:path*` → `API_BACKEND_URL ?? https://api-dev.usezombie.com`. Browser fetches never cross-origin → no CORS in dev/preview/prod. Server Components continue calling the backend directly. `API_BACKEND_URL` is server-only; `NEXT_PUBLIC_API_URL` controls display-only URLs.

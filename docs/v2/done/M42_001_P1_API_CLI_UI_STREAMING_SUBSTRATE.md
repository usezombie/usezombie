...
| Test | Asserts |
|---|---|
| `test_event_lifecycle_three_tables` | DONE — `event_loop_lifecycle_test.zig` (`integration: one processed event lands exactly 1 zombie_events + 1 telemetry + 1 zombie_sessions row`). Drives the full writepath via the harness binary, then SELECT-counts each of the three tables and asserts row count = 1 per join key, status='processed', response_text='ok', and `zombie_sessions.checkpoint_at > 0`. |
| `test_steer_returns_event_id` | `POST /steer` body must include `event_id` matching the XADD result |
| `test_event_replay_idempotent` | DONE — `event_loop_lifecycle_test.zig` (`integration: replaying the same event_id twice does not duplicate any of the three rows`). Drives writepath with a real ZombieEvent, then synthesizes a second ZombieEvent carrying the identical event_id (mirroring XAUTOCLAIM redelivery) and runs writepath again. Asserts: still 1 row in `core.zombie_events`, 1 in `zombie_execution_telemetry` (the schema's `ON CONFLICT (event_id) DO NOTHING` + UNIQUE constraint hold under replay), 1 in `core.zombie_sessions`. Found-and-fixed leak: `parseSessionContext` was returning `parsed.value` and dropping the arena handle, leaking a parse buffer on every event after the first. |
...
| `test_steer_cli_batch_roundtrip` | DONE — `zombiectl/test/zombie_steer_batch_roundtrip.unit.test.js`. Mocks `request` for the POST/steer (returns 202 + event_id) and the injectable `streamGet` to play scripted SSE frames (event_received → tool_call_started → 2× chunk → tool_call_completed → event_complete). Asserts: `[claw] <text>` printed for each chunk; terminal `event ${event_id} processed` printed; exit 0. Plus regression cases for agent_error → exit 1, missing message → exit 2, missing event_id in response → exit 1, frames whose event_id doesn't match are filtered out. The wire-level half (worker → executor RPC → activity PUBLISH) is asserted in the harness-binary integration test. |
| `test_steer_cli_interactive_history` | DEFERRED — interactive REPL is officially deferred per §5 decision 5. `commandSteer` returns exit 2 with a "interactive steer is not yet implemented" message when called without a message arg; that boundary is covered by the missing-message test in the batch-roundtrip suite. Test row will land when interactive REPL ships in a follow-up spec. |
| `test_events_cli_pagination` | DONE — `zombiectl/test/zombie_events_pagination.unit.test.js`. 50 + cursor + 10 split across two paginated `request` calls, asserting (1) first call carries `limit=50` and no `cursor=`, (2) follow-up call carries `cursor=<token>`, (3) the 50-row page renders the next-cursor hint and the 10-row tail does not. Plus coverage for `--actor` / `--since` flag forwarding, `--json` mode bypassing human format, and the empty-response path printing "No events yet." |
| `test_sse_publish_latency_p95_lt_200ms` | DONE — `sse_streaming_integration_test.zig`. In-process TestHarness boots the SSE handler on an ephemeral port; raw-TCP `SseClient` opens `GET /events/stream` with a Bearer JWT; 50 publish→receive trials (3 warmups + 50 measured) compute p95 from `nanoTimestamp()` deltas; assertion fails if p95 ≥ 200 ms. Test count reduced from spec's 100 to 50 — wall-clock budget on CI is tight and 50 is statistically sufficient for the p95 SLA. |
| `test_sse_reconnect_backfills_via_events` | DONE — `sse_streaming_integration_test.zig`. Four-phase choreography: (1) seed event A as a durable `zombie_events` row + publish its frame to SSE conn #1, (2) close the SSE socket with SO_LINGER=0 so the server's next write fails with EPIPE and frees its httpz worker, (3) seed events B + C while no SSE subscriber is connected (pub/sub frames lost; durable rows survive), (4) backfill via `GET /events?since=1h` and assert B + C appear, then reopen SSE conn #2 and confirm a frame for event D arrives live. Operator-visible state has no gaps. |
| `test_pubsub_failure_does_not_block` | DEFERRED to `docs/v2/pending/M42_002_P2_API_REDACTION_HARNESS.md` (extended scope). The fault model the spec asks for — PUBLISH fails but XADD/XACK still succeed on the same connection — has no clean injection point: Redis exposes per-command failures only via ACLs (heavy infra) or per-connection state, and adding a test-only flag to `queue_redis.Client` was rejected as production-code pollution. Invariant 7 ("durable record is independent of pub/sub") is enforced by the `catch` clauses in `activity_publisher.publish*` (publishRaw, publishEventReceived, publishToolCallStarted, publishChunk, publishToolCallProgress, publishToolCallCompleted, publishEventComplete) — five-line discipline easy to grep at review time. The follow-up spec will land an ACL-based test that uses a Redis user without `+publish` permission. |
| `test_args_redacted_no_secret_leak` | DEFERRED to follow-up spec (production-binary redaction harness). The `runner_progress.Adapter` redaction code path is **comptime-stripped** in the test harness binary, so a meaningful test requires running `zombied-executor` (NullClaw included) with a stub LLM provider that emits a synthetic tool_use event. RpcRecorder fixture (`src/zombie/test_rpc_recorder.zig`) is in place for the assertion side. Sanity-checked end-to-end via `event_loop_harness_recorder_test.zig` which proves the recorder captures wire bytes correctly using a script-supplied placeholder. |
...
| `test_rpc_version_mismatch_fast_fails` | DONE — `event_loop_harness_version_test.zig`. Harness binary spawned with `EXECUTOR_HARNESS_RPC_VERSION=1`; transport.Client at v2 connects and fails with `RpcVersionMismatch` in both directions (server sees v2 from client, client sees v1 from server). Both sides log `executor.rpc_version_mismatch`. Test asserts elapsed < 1.5 s (SLA is 100 ms; wall time ≪ that in practice). Implementation: `transport.setHelloVersionOverride` test-only setter, populated from env var only in harness builds (`build_options.executor_harness`). |
...
| `test_sse_sequence_resets_on_reconnect` | DONE — `sse_streaming_integration_test.zig`. Connection #1 receives frames id=0 then id=1; client closes (SO_LINGER=0 RST + drain publish wake); connection #2 reconnects with a forged `Last-Event-ID: 99` header; first frame on the new connection is asserted to have `id=0`, proving the handler ignores the request header and the per-conn sequence resets. |
| `test_executor_progress_callbacks_emit` | DONE — `event_loop_harness_integration_test.zig` (`integration: harness emits started/chunk/completed frames in order via real worker pipeline`). Spawned `zombied-executor-harness`, scripted 2 tool_call_started + 5 chunks + 2 tool_call_completed; subscriber on `zombie:{id}:activity` observes the same sequence (plus event_received/complete bookends), and `core.zombie_events.status` lands `processed`. |
| `test_executor_args_redacted_at_sandbox_boundary` | DEFERRED to follow-up spec (same reason as `test_args_redacted_no_secret_leak`). Requires production binary + stub LLM provider so `runner_progress.Adapter` redaction logic actually runs. Recorder fixture is ready for the byte-level assertion. |

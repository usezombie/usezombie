...
3. **Persisted creds win over default** — `creds.api_url=Z` with no flag/env resolves to `Z`. Enforced by `api-url-resolution.integration.test.js: honors creds.api_url when no flag and no env override are set`. (Was previously claimed as "enforced by existing test"; the existing test pinned `parseGlobalArgs` only, not the full chain through `cli.js:76`. Surfaced as a real bug during PR #297 review — see Discovery.)
...
- **Precedence-chain dead-code bug surfaced in PR review (caught + fixed in this PR).** Reviewer noted that this spec's invariant #3 ("Persisted creds win over default") was claimed but not test-pinned, only inferred from code structure. They were right: `parseGlobalArgs` (`args.js:79`) used to bake `DEFAULT_API_URL` into its return via `normalizeApiUrl(... || DEFAULT_API_URL)`, so `global.apiUrl` was always a truthy non-empty string. That short-circuited `creds.api_url` in `cli.js:76`'s `||` chain — the entire "sticky per-install" leg was dead code masquerading as documented behavior. A customer who ran `zombiectl login --api http://localhost:3000` would have their URL written to `credentials.json`, but every subsequent invocation silently fell through to the production default. **Fix:** `parseGlobalArgs` now returns `apiUrl: null` when nothing explicit is set, letting `cli.js:76` correctly cede to `creds.api_url` before landing on the default. Default-is-production claim moves from the parse-layer test to the integration layer (`api-url-resolution.integration.test.js`). Regression test pins the chain end-to-end: pre-seed `creds.api_url=http://localhost:3000`, run `zombiectl doctor` with empty env, assert `fetchImpl` is called with `http://localhost:3000/healthz`.

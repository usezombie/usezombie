...
Execute deployment from chronological playbooks (`M1_001` → `M2_001` → `M2_002` → `M3_001` → `M3_002`) and record deterministic evidence.
...
- 1.2 PENDING Must implement DEV data-plane provisioning automation from `docs/M2_002_PLAYBOOK_PRIMING_INFRA.md` (DB roles/migrations, Redis stream bootstrap via `XGROUP CREATE`)
- 1.3 IN_PROGRESS Container deployment path unblocked: `Dockerfile` refactored to binary-copy model; `deploy-dev.yml` wired (main push → GHCR → Railway DEV → /healthz poll → QA smoke → Discord notify); `release.yml` now has real PROD deploy steps (API verification + worker rollout over Tailscale SSH).
...
- 2.2 ✅ DONE Replaced `deploy-prod` stubs in `.github/workflows/release.yml` with real API + worker deploy: Railway PROD rollout verification (`/healthz` + `/readyz`) and worker rollout over Tailscale SSH using per-node keys from `op://${{ vars.VAULT_PROD }}/zombie-worker-{ant,bird}/ssh-private-key`
...
Treat deployment execution as validation of chronological playbook correctness and completeness.

**Dimensions:**
- 3.1 ✅ DONE Ambiguity/drift log captured in the milestone playbooks with concrete mismatches and correction policy
- 3.2 ✅ DONE Rebuilt deployment docs into chronological no-drift execution flow (`M1_001`, `M2_001`, `M2_002`, `M3_001`, `M3_002`)
...
- ✅ `scripts/check-credentials.sh` — portable credential gate for all vault items; DEV vault fully green, PROD missing 3 items (`planetscale-prod`, `tailscale/authkey`, `zombie-worker-{ant,bird}/ssh-private-key`).
...
- ✅ Replace `deploy-prod` echo stubs in `release.yml` with real Railway PROD verification + OVHCloud worker rollout via Tailscale SSH and per-node keys
- Keep `deploy-dev.yml` as main-push DEV lane; keep `release.yml` as tag-based PROD lane
...
- ✅ Finalize guide drift fixes with chronological milestone playbooks and shared sync policy

...
- 1.5 IN_PROGRESS D7 Backoff standardization with full `Retry-After` plumbing
- 1.6 PENDING D9 Error-context logging consistency across all critical boundaries
- 1.7 IN_PROGRESS D10 Error classification harmonization (worker + API)
...
**Dimensions:**
- 2.1.1 IN_PROGRESS Complete retry/error-classification coverage for all external side effects

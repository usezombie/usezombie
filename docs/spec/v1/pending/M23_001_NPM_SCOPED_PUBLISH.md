...
**Dimensions:**
- 1.1 PENDING `zombiectl/package.json` `name` field set to `@usezombie/zombiectl`
- 1.2 PENDING `zombiectl/README.md` install snippet updated to `npm install -g @usezombie/zombiectl`
- 1.3 PENDING Root `README.md` quick-start snippet updated to `npm install -g @usezombie/zombiectl`
- 1.4 PENDING `bun run build` passes after rename (no module resolution breakage)
...
**Dimensions:**
- 2.1 PENDING `release.yml` npm job uses `NODE_AUTH_TOKEN` from `op://ZMB_CD_PROD/npm-publish-token/credential`
- 2.2 PENDING `npm publish --provenance --access public --tag next` is the publish command
- 2.3 PENDING `id-token: write` permission present on the npm job (required for provenance)
- 2.4 PENDING After a release tag, `npm view @usezombie/zombiectl dist-tags` shows `next: 0.3.x`
...
**Dimensions:**
- 3.1 PENDING Promotion command documented: `npm dist-tag add @usezombie/zombiectl@<ver> latest`
- 3.2 PENDING `release.yml` has a comment block explaining `next` vs `latest` tag lifecycle
- 3.3 PENDING `zombiectl/README.md` install snippet uses `@usezombie/zombiectl` (no tag pin — resolves `latest` by default)

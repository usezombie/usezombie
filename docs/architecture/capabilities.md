...
| Provider config (BYOK) | Current `main` stores BYOK credentials through the workspace-scoped `credentials/llm` route. The tenant-scoped `core.tenant_providers` posture described in [`billing_and_byok.md`](./billing_and_byok.md) is the pending M48 target contract. | BYOK Provider (M48) |

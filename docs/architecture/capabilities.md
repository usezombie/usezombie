...
| Provider config (BYOK) | The long-term posture is per-tenant choice between platform-managed inference and Bring Your Own Key. Current `main` stores BYOK credentials through the workspace-scoped `credentials/llm` route; the tenant-scoped `core.tenant_providers` posture described in [`billing_and_byok.md`](./billing_and_byok.md) is the architectural target contract. | Provider resolution path |

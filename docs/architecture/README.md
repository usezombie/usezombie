...
| [`scaling.md`](./scaling.md) | How the runtime grows on Upstash. Worked 100-zombie / 10-worker example with per-box and fleet-wide connection budgets, per-request volume math (the idle XREADGROUP bill), tuneup-knob table, growth paths, and failover-storm behavior. Read alongside `data_flow.md` §"Connection topology". |

...
| **NullClaw** | The language-model agent loop that runs inside the runner's sandboxed child — the "zombie's agent." [(more)](./capabilities.md#1-reasoning--tool-inventory-declared-in-the-zombies-own-files) |
...
| **zombie-runner** | The host-resident binary (the parent control loop + NullClaw execution linked in — no separate executor sidecar) that registers to `zombied` and pulls work; holds no datastore credentials. Implemented at the M80_002 cutover. [(more)](./runner_fleet.md) |
...
| **Stage** | One `runner.execute` call inside the runner's sandboxed child — one language-model context window's worth of reasoning. Long incidents span multiple stages via continuation events. [(more)](./capabilities.md#4-context-lifecycle--keeping-a-long-incident-reasoning-past-the-models-working-memory-limit) |

...
| **NullClaw** | The language-model agent loop that runs inside the runner's sandboxed child — the "agent's agent." [(more)](./capabilities.md#1-reasoning--tool-inventory-declared-in-the-agents-own-files) |
...
| **User's agent** | The workstation tool the human types into (Claude Code / Amp / Codex CLI / OpenCode) — drives `zombiectl`; distinct from the agent's agent. [(more)](./user_flow.md#§8.0-the-wedge-surface-usezombie-install-platform-ops-skill) |

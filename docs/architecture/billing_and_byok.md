...
- **Platform-managed (v2.0 default = Fireworks Kimi K2.6).** UseZombie holds the platform-side Fireworks key — `PLATFORM_FIREWORKS_KEY`, admin-primed in the platform vault (same M45 crypto_store path used for user credentials, at a platform-scope identifier). It loads into server-side process memory at API boot and is never echoed to any user-facing surface. The user pays UseZombie a per-event fee that bundles inference (token-based, retail-rate-driven through the model-caps endpoint) plus orchestration, storage, and egress.

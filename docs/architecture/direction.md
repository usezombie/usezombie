...
- **Destructive actions are never assumed safe just because the model suggested them.** The `approval_required` policy lives in `TRIGGER.md`; the runner enforces it before starting the stage. SKILL.md prose may ask for approval explicitly. See [`capabilities.md`](./capabilities.md) §3 "Approval gating".

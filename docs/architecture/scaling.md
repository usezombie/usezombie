...
| `SSE_MAX_STREAMS` | 64 | Concurrent SSE tails per replica — each is a dedicated detached thread (~0.5 MiB stack + 2 fds; budget math at the const in `runtime_loader.zig`). 0 is rejected at boot. | 503s on `GET /events/stream` (`zombie_sse_backpressure_rejections_total`) while the box has memory headroom; watch `zombie_sse_in_flight_streams`. |

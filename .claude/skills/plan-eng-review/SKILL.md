...
* Deep modules over shallow ones — files under 500 lines keep testing and review simpler.
* 500-line check is a **warning**, not an error — flag files approaching limit, recommend splitting.
...
* **File size** — flag any file approaching 500 lines; deep modules with fewer than 500 lines keep testing and review simpler. If a file exceeds 500 lines, recommend splitting into focused modules.
* File size — flag any file approaching 500 lines; consider splitting.
...
**Zig/React/Next.js test checks:**
* For each new API route: is there a Jest/Vitest test?
* For each new React component: are there React Testing Library tests?
* For each new Zig function: is there a Zig test (`test "name" { ... }`)?
* For each new hook: are edge cases (loading, error, empty) tested?

**STOP.** For each issue found in this section, call AskUserQuestion individually. One issue per call. Present options, state your recommendation, explain WHY. Do NOT batch multiple issues into one AskUserQuestion. Only proceed to the next section after ALL issues in this section are resolved.

### Test Plan Artifact

After producing the test diagram, write a test plan artifact to the project directory:

Write to `.context/test-plans/{branch}-test-plan.md`:
...
This file helps future test runs know what to verify.

### 4. Performance review
Evaluate:
* N+1 queries and database access patterns (React: check for unnecessary re-renders, missing `useMemo`/`useCallback`).
* Memory-usage concerns.
* Caching opportunities (Next.js: check for `getStaticProps`, `getServerSideProps`, ISR).
* Bundle size impact (Next.js: check for dynamic imports, code splitting).
* Zig: check for unnecessary allocations, memory leaks in hot paths.
* Slow or high-complexity code paths.
* Database query optimization for new queries.

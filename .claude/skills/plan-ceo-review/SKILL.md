...
grep -r "TODO\|FIXME\|HACK\|XXX" --include="*.py" -l
find . -name "*.py" -newer requirements.txt 2>/dev/null | head -20  # Recently touched files
```
...
```
  ...
  -------------------------|-----------------------------|-----------------
  ExampleService#call      | API timeout                 | requests.Timeout
                           | API returns 429             | RateLimitError
                           | API returns malformed JSON  | json.JSONDecodeError
                           | DB connection pool exhausted| OperationalError
                           | Record not found            | DoesNotExist
  -------------------------|-----------------------------|-----------------

  EXCEPTION CLASS              | RESCUED?  | RESCUE ACTION          | USER SEES
  -----------------------------|-----------|------------------------|------------------
  requests.Timeout             | Y         | Retry 2x, then raise   | "Service temporarily unavailable"
  RateLimitError               | Y         | Backoff + retry         | Nothing (transparent)
  json.JSONDecodeError         | N ← GAP   | —                      | 500 error ← BAD
  OperationalError             | N ← GAP   | —                      | 500 error ← BAD
  DoesNotExist                 | Y         | Return None, log warning | "Not found" message
```
Rules for this section:
* `rescue StandardError` is ALWAYS a smell. Name the specific exceptions.
* `except Exception as e` with only `logger.error(e.message)` is insufficient. Log the full context: what was being attempted, with what arguments, for what user/request.
...
Evaluate:
* N+1 queries or unnecessary re-renders. For React: are props memoized? For Zig: are allocations minimized?
* Memory usage. For every new data structure: what's the maximum size in production?
* Bundle size. For new frontend code: impact on initial load? Code splitting needed?
...
* Admin tooling. New operational tasks that need admin UI or management commands?
...
* Ecosystem fit. Aligns with Zig/React/Next.js ecosystem direction?

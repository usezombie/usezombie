...
**If there are merge conflicts:** Try to auto-resolve if they are simple (VERSION, migrations, CHANGELOG ordering). If conflicts are complex or ambiguous, **STOP** and show them.
...
**Zig:**
```bash
zig build test 2>&1 | tee /tmp/ship_tests.txt
```
...
## Test plan
- [x] All pytest tests pass (N tests, 0 failures)

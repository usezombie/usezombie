...
#### SQL & Data Safety
- String interpolation in SQL (even if values are cast — use parameterized queries via ORM or `cursor.execute(sql, params)`)
- TOCTOU races: check-then-set patterns that should be atomic `F()` expressions or `select_for_update()`
- `.save(update_fields=[...])` bypassing other validations, or direct `.update()` on fields that have signals/constraints
- N+1 queries: `select_related()` / `prefetch_related()` missing for FK/M2M relationships used in loops (especially in DRF serializers)

#### Race Conditions & Concurrency
- Read-check-write without uniqueness constraint or `IntegrityError` handling (e.g., `filter().first()` then `save()` without handling concurrent insert)
- `get_or_create()` on columns without unique DB index — concurrent calls can create duplicates
- Status transitions that don't use atomic `F()` or conditional `update(status=new, status=old)` — concurrent updates can skip or double-apply transitions
- User-controlled data in API responses without sanitization (XSS in DRF serializers — ensure `SafeText` or auto-escaping in templates)
...
- `random.random()` / `random.randint()` for security-sensitive values — use `secrets` module instead
...
- O(n*m) lookups in templates/views (`for` loops in nested templates instead of dict lookups)
- Python-side list comprehension filtering on QuerySets that could be a `filter()` clause (unless intentionally avoiding heavy DB queries)

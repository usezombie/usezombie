...
CREATE INDEX idx_agent_config_versions_agent ON agent_config_versions(agent_id, version DESC);
CREATE INDEX idx_agent_config_versions_tenant ON agent_config_versions(tenant_id, created_at DESC);

CREATE TABLE workspace_active_config (
...
CREATE TABLE config_compile_jobs (
    ...
    tenant_id             UUID NOT NULL REFERENCES tenants(tenant_id),
    workspace_id          UUID NOT NULL REFERENCES workspaces(workspace_id) ON DELETE CASCADE,
    requested_agent_id    UUID NOT NULL REFERENCES agent_profiles(agent_id) ON DELETE CASCADE,
...
CREATE INDEX idx_config_compile_jobs_workspace ON config_compile_jobs(workspace_id, created_at DESC);
CREATE INDEX idx_config_compile_jobs_tenant ON config_compile_jobs(tenant_id, created_at DESC);

...
CREATE POLICY agent_profiles_update_tenant ON agent.agent_profiles
    FOR UPDATE USING (tenant_id::text = current_setting('app.current_tenant_id', true))
    WITH CHECK (tenant_id::text = current_setting('app.current_tenant_id', true));
CREATE POLICY agent_profiles_delete_tenant ON agent.agent_profiles
    FOR DELETE USING (tenant_id::text = current_setting('app.current_tenant_id', true));

CREATE POLICY agent_config_versions_select_tenant ON agent.agent_config_versions
    FOR SELECT USING (tenant_id::text = current_setting('app.current_tenant_id', true));
CREATE POLICY agent_config_versions_insert_tenant ON agent.agent_config_versions
    FOR INSERT WITH CHECK (tenant_id::text = current_setting('app.current_tenant_id', true));
CREATE POLICY agent_config_versions_update_tenant ON agent.agent_config_versions
    FOR UPDATE USING (tenant_id::text = current_setting('app.current_tenant_id', true))
    WITH CHECK (tenant_id::text = current_setting('app.current_tenant_id', true));
CREATE POLICY agent_config_versions_delete_tenant ON agent.agent_config_versions
    FOR DELETE USING (tenant_id::text = current_setting('app.current_tenant_id', true));

CREATE POLICY workspace_active_config_select_tenant ON agent.workspace_active_config
    FOR SELECT USING (tenant_id::text = current_setting('app.current_tenant_id', true));
CREATE POLICY workspace_active_config_insert_tenant ON agent.workspace_active_config
    FOR INSERT WITH CHECK (tenant_id::text = current_setting('app.current_tenant_id', true));
CREATE POLICY workspace_active_config_update_tenant ON agent.workspace_active_config
    FOR UPDATE USING (tenant_id::text = current_setting('app.current_tenant_id', true))
    WITH CHECK (tenant_id::text = current_setting('app.current_tenant_id', true));
CREATE POLICY workspace_active_config_delete_tenant ON agent.workspace_active_config
    FOR DELETE USING (tenant_id::text = current_setting('app.current_tenant_id', true));

CREATE POLICY config_compile_jobs_select_tenant ON agent.config_compile_jobs
    FOR SELECT USING (tenant_id::text = current_setting('app.current_tenant_id', true));
CREATE POLICY config_compile_jobs_insert_tenant ON agent.config_compile_jobs
    FOR INSERT WITH CHECK (tenant_id::text = current_setting('app.current_tenant_id', true));
CREATE POLICY config_compile_jobs_update_tenant ON agent.config_compile_jobs
    FOR UPDATE USING (tenant_id::text = current_setting('app.current_tenant_id', true))
    WITH CHECK (tenant_id::text = current_setting('app.current_tenant_id', true));
CREATE POLICY config_compile_jobs_delete_tenant ON agent.config_compile_jobs
    FOR DELETE USING (tenant_id::text = current_setting('app.current_tenant_id', true));

...
pub fn NonBlocking(comptime S: type, comptime WSH: type) type {
    return struct {
        ...
        pub fn deinit(self: *Self) void {
            ...
            // Stop pool BEFORE freeing anything it may reference. deinit only
            // releases the arena; without stop, pool workers blocked in
            // read_cond.wait can wake on a freed queue, or queued
            // processData(conn,...) tasks fire on Conns freed below.
            // Symptom: SIGSEGV in thread_pool.zig:getNext "queue[tail]" or
            // worker.zig:processData "switch (conn.protocol)" at shutdown.
